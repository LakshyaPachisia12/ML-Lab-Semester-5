...
def get_entropy_of_dataset(tensor: torch.Tensor):
    ...
    # Step 1: Count occurences of each class
    ...
    # Step 4: Return the entropy value
...
def get_avg_info_of_attribute(tensor: torch.Tensor, attribute: int):
    ...
    # Step 1: Get unique values and their counts for the attribute
    attr_values, attr_counts = torch.unique(tensor[:, attribute], return_counts=True)

    # Step 2: Set weighted entropy as 0
    weighted_entropy = 0.0

    # Step 3: Run a for loop to iterate through all the values of the attributes and find the entropy of each attribute
    for value, count in zip(attr_values, attr_counts):
        subset = tensor[tensor[:, attribute] == value]
        subset_entropy = get_entropy_of_dataset(subset)
        weighted_entropy += (count.float() / attr_counts.sum()) * subset_entropy
...
def get_information_gain(tensor: torch.Tensor, attribute: int):
    ...
    attribute_avg_info = get_avg_info_of_attribute(tensor, attribute)

    info_gain = dataset_entropy - attribute_avg_info
...
def get_selected_attribute(tensor: torch.Tensor):
    ...
    for attr in range(tensor.shape[1] - 1):
        info_gains[attr] = get_information_gain(tensor, attr)
    best_attr = max(info_gains, key=info_gains.get)             #type:ignore
